...
def _is_percent_col(colname: str) -> bool:
    """
    Percent columns: any with 'yoy' or 'surprise %' in name.
    """
    c = str(colname).lower()
    return ("yoy" in c) or ("surprise %" in c)
...
def render_earnings_momentum_section(
    ...
    color_extra = [c for c in ["SUE (EPS)", "Beat/Miss Streak"] if c in display.columns]
